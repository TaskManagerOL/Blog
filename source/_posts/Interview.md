...
+ 强缓存和协商缓存的协议是怎么样的？
  ...
  修正：
  	...
  	强缓存：
  	依据缓存控制相关指令直接判定是否使用本地缓存资源，无需向服务器验证资源是否更新，常见的控制指令有Cache-Control: max-age和Expires等。
  	协商缓存：
  	当强缓存失效后，浏览器会携带缓存标识向服务器发起请求，由服务器根据请求中的标识判断资源是否可用，常见的协商缓存机制有Last-Modified/If-Modified-Since和ETag/If-None-Match等。
  ```
